...
const VALID_SPELLS = new Set((spells as SpellType[]).map((s) => s.id as SpellId));
...
export default function Classpage() {
	...
	const { classId, spellId } = useParams<{ classId?: ClassId; spellId?: SpellId }>();
	...
	const [highlightedClass, setHighlightedClass] = useState<ClassId | undefined>(undefined);
	...
	if (spellId) {
		...
		if (classId && !CLASS_TO_SPELLS.get(classId)?.has(spellId)) return <NotFound />;
	}
